...
export async function authorize(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    if (request.method === 'GET') {
      ...
      const body = `<!doctype html><html lang="ja"><meta charset="utf-8"><title>Gemini Connect for ChatGPT 接続許可</title><h1>Gemini Connect for ChatGPT 接続許可</h1><p>このクライアントにGeminiの利用を許可します。依頼内容はGoogleへ送信され、APIのバックグラウンド実行と結果取得のため依頼・応答がGoogle側に保存されます。Gemini APIを使う場合は、このMCPの所有者が設定したAPIキーの利用枠・料金が使われます。</p><p>クライアント: ${escape(oauth.clientId)}</p><p>戻り先: ${escape(oauth.redirectUri)}</p><p>設定された所有者のGoogleアカウントだけ接続できます。</p><form method="post"><input type="hidden" name="csrf" value="${login.state}"><button type="submit">Googleでログインして接続を許可</button></form></html>`;
