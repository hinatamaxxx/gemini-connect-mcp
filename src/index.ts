...
const api = {
  async fetch(request, env, ctx) {
    ...
    return createMcpHandler(() => {
      const server = new McpServer({ name: 'gemini-connect-for-chatgpt', title: 'Gemini Connect for ChatGPT', version: '1.7.0' }, { instructions: `
...
ChatGPTなどCodex以外はcli_modelsを省略してAPIを実行する。ローカルCLIを探さない。APIはバックグラウンド実行する。in_progressなら15秒以上空けてask_geminiにjob_idだけを渡し結果を取得する。受付済みpromptを再送して生成を開始し直さない。処理中は同じ受付番号で結果確認し、長引く場合はjob_idを保持して状況を伝える。poll_errorは同じjob_idで再確認できる。開始のタイムアウトは再送しない。usageは返った応答分のみで、失敗試行の課金を含む総額ではない。
` });
      server.registerTool('ask_gemini', {
        description: 'Geminiへ文章生成・分析・第二意見・創作・URL調査を依頼。writing_rules=trueで文章補正。既定は最新Flash・high。Codexはcli_modelsを指定してCLI用チケットを取得、その他は省略してAPI実行。入力はGoogleへ送信され課金され得る。APIは受付番号を返し、job_idで結果取得。依頼と応答をGoogle側に保存する。',
        inputSchema,
        ...
        try {
          if (input.cli_models && !input.job_id) {
          ...
          return { isError: output.status !== 'in_progress' && !output.text, content: [{ type: 'text', text: JSON.stringify(output) }], structuredContent: output };
        } catch (error) {
