...
HTTP ステータスコード 429、408、500、502、503、504、および接続エラーやタイムアウトが発生した場合は、最大2回まで再試行（合計3回試行）します。待機時間にはランダムな遅延（ジッター）を加算します。待機時間と API 処理時間を合わせた全体の制限時間は 110 秒、1試行あたりの上限は 60 秒です。残り時間が次の試行に足りない場合は、試行回数が残っていても終了します。SDK 側の自動再試行機能は無効化しています。

429の待機時間は15秒と30秒、その他の一時障害は1秒と2秒を基本とします。GoogleがRetry-AfterやRetryInfoで待機時間を返した場合は、その指定より早く再試行しません。待機時間が処理全体の残り時間に収まらない場合は再試行せず、retry_after_secondsを返します。Googleが日次利用枠の超過を明示した場合も再試行しません。エラー時にはsourceを返し、MCP側の回数制限とGemini API側のエラーを区別します。
...
Cloudflare Workers によるレート制限はエッジ拠点単位で適用されるため、全世界からのリクエストに対する厳密な課金上限を保証するものではありません。Gemini API は `store: false` で実行し、Interactions API で後から参照するための会話保存を無効にしています。ただし、Google 側のその他のデータ保持や利用条件まで無効にする設定ではありません。外部ウェブページ、文章ルール、モデルからの応答内容はすべて参考資料として扱い、プロンプトインジェクション（指示の乗っ取り）や未確認の出典を採用しないよう注意してください。
