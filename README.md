...
HTTP ステータスコード 429、408、500、502、503、504、および接続エラーやタイムアウトが発生した場合は、最大2回まで再試行（合計3回試行）します。待機時間は1秒、2秒を基本とし、ランダムな遅延（ジッター）を加算します。待機時間と API 処理時間を合わせた全体の制限時間は 110 秒、1試行あたりの上限は 60 秒です。残り時間が次の試行に足りない場合は、試行回数が残っていても終了します。SDK 側の自動再試行機能は無効化しています。
